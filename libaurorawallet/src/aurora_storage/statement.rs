...
impl Statement {
    pub fn as_str(&self) -> &str {
        match self {
            ...
            &Statement::GetRecordID => "SELECT id FROM items WHERE type = :type AND name = :name",
            &Statement::DeleteRecord => "DELETE FROM items WHERE wallet_id = :wallet_id AND type = :type AND name = :name",
            &Statement::UpdateRecordValue => "UPDATE items SET value = :value WHERE type = :type AND name = :name",
            &Statement::CheckRecordExists => "SELECT COUNT(id) FROM items WHERE type = :type AND name = :name",
