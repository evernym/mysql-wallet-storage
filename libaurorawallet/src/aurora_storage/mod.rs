...
impl<'a> AuroraStorage<'a> {
    ...
    pub fn search_records(&self, type_: &str, query_json: &str, options_json: &str, search_handle_p: *mut i32) -> ErrorCode {
        ...
        let (query, arguments) = query_translator::wql_to_sql(self.wallet_id, type_, &wql, &fetch_options);
    ...
    pub fn fetch_search_next_record(&self, search_handle: i32, record_handle_p: *mut i32) -> ErrorCode {
        ...
        let next_result = match search_result.next() {
            Some(v) => v,
            _ => {
                self.free_search(search_handle);
        ...
        let record_type: String = check_option!(row.get(0), ErrorCode::IOError);
        let record_id: String = check_option!(row.get(1), ErrorCode::IOError);
        let record_value: Vec<u8> = check_option!(row.get(2), ErrorCode::IOError);
        let record_tags: String = check_option!(row.get(3), ErrorCode::IOError);

        let record = Record::new(
            check_result!(CString::new(record_id), ErrorCode::InvalidState),
            if !record_value.is_empty() {Some(record_value)} else {None},
            if record_tags != "" {Some(check_result!(CString::new(record_tags), ErrorCode::InvalidState))} else {None},
            if record_type != "" {Some(check_result!(CString::new(record_type), ErrorCode::InvalidState))} else {None},
        );
