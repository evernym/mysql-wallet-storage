...
use mysqlstorage::utils::multi_pool::{StorageConfig, StorageCredentials};
use std::cmp::max;
use std::thread;
...
const RECORDS_PER_WALLET_CNT: u64 = 10;
    ...
    ///
    /// Enum representing all mysql api requests
    ///
    enum Action {
        AddWallet,
        SetMetadata,
    ...
    ///
    /// Defines steps that should be executed for each mysql wallet api request.
    ///
    ///
    /// # Arguments
    ///
    ...
    ///
    /// Sends mysql api requests in parallel and calculate time needed for execution.
