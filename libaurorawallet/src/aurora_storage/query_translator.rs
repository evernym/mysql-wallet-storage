...
fn operator_to_sql(op: &Operator, arguments: &mut Vec<Value>) -> String {
    match *op {
...
fn gt_to_sql(tag_name: &String, tag_value: &String, arguments: &mut Vec<Value>) -> String {
    match tag_name.chars().next().unwrap_or('\0') {
        '~' => {
            arguments.push(tag_name[1..].into());
            arguments.push(tag_value.into());
...
fn gte_to_sql(tag_name: &String, tag_value: &String, arguments: &mut Vec<Value>) -> String {
    match tag_name.chars().next().unwrap_or('\0') {
        '~' => {
            arguments.push(tag_name[1..].into());
            arguments.push(tag_value.into());
...
fn lt_to_sql(tag_name: &String, tag_value: &String, arguments: &mut Vec<Value>) -> String {
    match tag_name.chars().next().unwrap_or('\0') {
        '~' => {
            arguments.push(tag_name[1..].into());
            arguments.push(tag_value.into());
...
fn lte_to_sql(tag_name: &String, tag_value: &String, arguments: &mut Vec<Value>) -> String {
    match tag_name.chars().next().unwrap_or('\0') {
        '~' => {
            arguments.push(tag_name[1..].into());
            arguments.push(tag_value.into());
...
fn like_to_sql(tag_name: &String, tag_value: &String, arguments: &mut Vec<Value>) -> String {
   match tag_name.chars().next().unwrap_or('\0') {
        '~' => {
            arguments.push(tag_name[1..].into());
            arguments.push(tag_value.into());
...
fn regex_to_sql(tag_name: &String, tag_value: &String, arguments: &mut Vec<Value>) -> String {
    match tag_name.chars().next().unwrap_or('\0') {
        '~' => {
            arguments.push(tag_name[1..].into());
            arguments.push(tag_value.into());
...
fn and_to_sql(suboperators: &[Operator], arguments: &mut Vec<Value>) -> String {
    join_operators(suboperators, " AND ", arguments)
}

fn or_to_sql(suboperators: &[Operator], arguments: &mut Vec<Value>) -> String {
    join_operators(suboperators, " OR ", arguments)
}

fn not_to_sql(suboperator: &Operator, arguments: &mut Vec<Value>) -> String {
    let suboperator_string = operator_to_sql(suboperator, arguments);
    "NOT (".to_string() + &suboperator_string + ")"
}

fn join_operators(operators: &[Operator], join_str: &str, arguments: &mut Vec<Value>) -> String {
...
pub fn wql_to_sql(wallet_id: u64, type_: &str, wql: &Operator, options: &FetchOptions) -> (String, Vec<Value>) {
    let mut arguments: Vec<Value> = Vec::new();
    let query_condition = operator_to_sql(wql, &mut arguments);

    let query_string = format!(
        "SELECT {}, name, {}, {} FROM items i WHERE {} AND i.type = ? AND wallet_id = ?;",
        if options.fetch_type { "type" } else {"''"},
